...
exports.response = function (path, options, request, _preloaded) {
    ...
    if (options.confine) {
        ...
        path = Hoek.isAbsolutePath(path) ? Path.normalize(path) : Path.join(confineDir, path);
        ...
        path = Hoek.isAbsolutePath(path) ? Path.normalize(path) : Path.join(request.route.settings.files.relativeTo, path);
    }
